##cell_type_of_origin_by_lincrna.R
##by: Rocco Lucero
##started: July 3, 2017
...
##Select lincrnatable
entex_lincrna <- (sep.df.by.colname.pttn(t(exprs.gencode.rpm)))[['lincRNA']]

##create metadata##
entex_meta_all <- make.meta.from.entex.names(t(entex_lincrna))
...
##subset entex reads##
keep_lincrna <- !(apply(X = entex_lincrna, MARGIN = 2, FUN = filterBySD, min_sd = min_sd))
table(keep_lincrna)

lincrna_adult <- entex_lincrna[adult_samples, keep_lincrna]
rownames(lincrna_adult) <- rownames(entex_meta_adult)
lincrna_adult <- lincrna_adult[samp_subset, ]
entex_meta_adult <- entex_meta_adult[samp_subset, ]

tissue_refs_nrmlzd <- get.normalized.readcounts.obj(lincrna_adult)
...
exrna_atlas_rpm <- readRDS("../get_atlas_data_in_R/interim/exrna_atlas_readcounts_non_gencode.RDS")
exrna_atlas_meta <- readRDS(file = '../cell_type_of_origin/input/comprehensive_atlas_metadata_draft_1.RDS')

#select lincrna table
atlas_lincrna <- exrna_atlas_rpm$piRNA
...
atlas_lincrna <- atlas_lincrna[blood_samples,]

##subset atlas reads and normalize##
keep_lincrna <- !(apply(X = atlas_lincrna, MARGIN = 2, FUN = filterBySD, min_sd = min_sd))
table(keep_lincrna)

atlas_lincrna <- atlas_lincrna[ , keep_lincrna]

atlas_lincrna_nrmlzd <- get.normalized.readcounts.obj(atlas_lincrna)


##tidy the global environment##
rm_dat <- grep(pattern = "exogenous", value = T, x = ls())
do.call(rm, as.list(rm_dat))
rm(exrna_atlas_gencode)
...
atlas_blood_lincrna <- atlas_lincrna_nrmlzd$qn.npn.lgstc
atlas_blood_meta <- droplevels.data.frame(exrna_atlas_meta[blood_fractions,])



avail_probes <- intersect(colnames(tiss_refs), colnames(atlas_blood_lincrna))
tiss_refs <- tiss_refs[, avail_probes]
atlas_blood_lincrna <- atlas_blood_lincrna[, avail_probes]
...
cors_tiss_lincrna <- cor(x = t(tiss_refs))
heatmap.2(cors_tiss_lincrna,
                  trace="none",
...
#This demonstrates that some tissues have consistent and distinct lincrna signatures
...
my_probes <- intersect(colnames(atlas_blood_lincrna), union(markers_ep, markers_ovr))
...
####################################################################################################
atlas_rc_subset <- atlas_blood_lincrna
...
atlas_blood_pls_refs_lincrna <- rbind(atlas_blood_lincrna, spike_in)

atlas_rc_subset <- atlas_blood_pls_refs_lincrna
