...
avail_probes <- intersect(colnames(tiss_refs), colnames(atlas_blood_mirna))
tiss_refs <- tiss_refs[, avail_probes]
atlas_blood_mirna <- atlas_blood_mirna[, avail_probes]
...
age_colors <- as.factor(round(as.numeric(atlas_blood_meta$Age)/20,0))
levels(age_colors) <- rainbow(length(levels(age_colors)))
age_colors <- as.character(age_colors)

disease_colors <- as.factor(atlas_blood_meta$Disease.Type)
levels(disease_colors) <- rainbow(length(levels(disease_colors)))
disease_colors <- as.character(disease_colors)

study_colors <- as.factor(atlas_blood_meta$Study)
levels(study_colors) <- rainbow(length(levels(study_colors)))
study_colors <- as.character(study_colors)

library_colors <- as.factor(atlas_blood_meta$Library.Construction.Kit)
levels(library_colors) <- rainbow(length(levels(library_colors)))
library_colors <- as.character(library_colors)

isolation_colors <- as.factor(atlas_blood_meta$RNA.Isolation.Kit)
levels(isolation_colors) <- rainbow(length(levels(isolation_colors)))
isolation_colors <- as.character(isolation_colors)

donor_type_colors <- as.factor(atlas_blood_meta$Donor.Type)
levels(donor_type_colors) <- rainbow(length(levels(donor_type_colors)))
donor_type_colors <- as.character(donor_type_colors)


################################################################################
if(T){
# Compute correlation matrix and draw a heatmap
cors_tiss_mirna <- cor(x = t(tiss_refs))
...
#This demonstrates that some tissues have consistent and distinct miRNA signatures
