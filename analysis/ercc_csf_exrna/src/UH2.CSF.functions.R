...
clear.templates <- function(){
    ...
    RNA.type.lst <- list(circRNA.sense = NULL, circRNA.anti = NULL,
                         gencode.sense = NULL, gencode.anti = NULL,
                         gencode.sense.gene = NULL, gencode.anti.gene = NULL,
                         miRNA.mature = NULL, miRNA.pre.sense = NULL, miRNA.pre.anti = NULL,
                         tRNA.sense = NULL, tRNA.anti = NULL,
                         piRNA.sense = NULL, piRNA.anti = NULL)
    ...
    for(cur.kit in names(kits.lst)){
        ...
        for(cur.cond in names(bio.conditions.lst)){
            ...
            for(cur.rna.ty in names(RNA.type.lst)){
                cur.rna.ty.1 <- gsub(pattern = '\\.',replacement = '.*',cur.rna.ty)
                ptt <- paste('.*',cur.rna.ty.1,'.*',sep = '')
                pth <- paste(df.path,dat.fldr,sep = '')
                
                f.cur <- list.files(path = pth,pattern = ptt)
